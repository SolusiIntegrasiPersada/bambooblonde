...
class XlsxBestSellerModelStore(models.Model):
    _name = 'report.sol_bb_report.best_seller_model_store.xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, objs):
        header_table = ['Model', 'Category', 'Style', 'Stock Name', 'Stock ID', 'Color', 'Aging', 'Barcode', 'Size', 'Order Notes', 'Cost Price', 'Retail Price', 'Qty Sold', 'In Stock', 'In Stock']
        ...
        product_model_id = self.env['product.model'].sudo().browse(datas.get('product_model_id'))
        ...
        pos_order_line = self.env['pos.order.line'].sudo().search([
            ...
            ('order_id.session_id.config_id', '=', datas.get('pos_config_id')),
            ('order_id.state', 'in', ['paid', 'done', 'invoiced']),
        ])

        product_tmpl_ids = self.env['product.template'].sudo().search([
            ('active', '=', True),
            ('product_model_id', '=', datas.get('product_model_id'))
        ])
        ...
        row += 1
        sheet.merge_range(row, 0, row, len(header_table)-1, f'{pos_order_line.mapped("order_id.picking_type_id.warehouse_id.name")[0]}', formatNormal)
        row += 1
        sheet.merge_range(row, 0, row, len(header_table)-1, f'{pos_order_line.mapped("order_id.picking_type_id.warehouse_id.partner_id.contact_address_complete")[0]}', formatNormal)
        row += 1
        sheet.merge_range(row, 0, row, len(header_table)-1, f'BEST SELLER {product_model_id.name}', formatNormal)
        row += 1
        ...
        sheet.merge_range(row, column-3, row, column-2, 'TOTAL SALES & STOCK', formatHeaderTableSand)
        sheet.write(row, column-1, 'WH STOCK', formatHeaderTableSand)
