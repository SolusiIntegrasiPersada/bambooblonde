...
header_table = ['No', 'Picture', 'New-Re Order', 'Del Date', 'Po Bamboo', 'PO Taboo', 'Style', 'Fabric', 'Color',
                'Size', 'Qty', 'Cost Price', 'Total', 'Trans Date', 'Supplier', 'Date Out', 'Date In', 'Status']
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        for pc_id in list_categ_ids:
            ...
            sheet.merge_range(row, 0, row, len(header_table) - 1,
                              f'NEW ORDER & RE-ORDER BAMBOO {from_date} - {to_date}', formatHeaderCompany)
            ...
            no = 1
            if datas.get('order_type') != 'all':
                mrp_ids = mrp_ids.filtered(lambda l: l.purchase_id.order_type == datas.get('order_type'))
            ...
            for mrp in mrp_ids:
                ...
                picture = io.BytesIO(
                    base64.b64decode(mrp.product_tmpl_id.image_128)) if mrp.product_tmpl_id.image_128 else False
                ...
                fabric = mrp.purchase_id.order_line[0].fabric_por.name or '' if len(
                    mrp.purchase_id.order_line) > 0 else ''
                ...
                po_line_product = mrp.purchase_id.order_line.filtered(
                    lambda x: x.product_id.product_tmpl_id == mrp.product_tmpl_id)
                ...
                date_out = wo_ids[0].out_date.strftime('%d/%m/%Y') if wo_ids[0].out_date else '' if len(
                    wo_ids) > 0 else '' or ''
                date_in = wo_ids[0].in_date.strftime('%d/%m/%Y') if wo_ids[0].in_date else '' if len(
                    wo_ids) > 0 else '' or ''
                ...
                if picture:
                    ...
                    sheet.insert_image(row, 1, "image.png",
