...
class XlsxMensClothes(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        datas = data.get('form', {})
        dt_type_of_report = datas.get('type_in_report', False)
        ...
        row = 1
        if dt_type_of_report == 'best' :
            name_report = 'BEST SELLER'
        else :
            name_report = 'SLOW SELLER'
        if dt_is_stock_type:
            sheet.merge_range(row, 0, row, len(header_table_real_with_stock_type)-1, f'{name_report} {product_model_id.name} WITH STOCK TYPE', formatNormal)
            row += 1
            ...
            sheet.merge_range(row, 0, row, len(header_table_real)-1, f'{name_report} {product_model_id.name}', formatNormal)
            row += 1
        ...
        for categ in categ_ids:
            ...
            if dt_type_of_report == 'best' :
                number = categ.number_of_best_product
            else :
                number = categ.number_of_slow_product
            number_of_best_product = number
            if dt_is_stock_type:
                if raw_data:
...
class XlsxSlowClothes(models.Model):
    _name = 'report.sol_bb_report.slow_clothes.xlsx'
    _inherit = 'report.report_xlsx.abstract'
