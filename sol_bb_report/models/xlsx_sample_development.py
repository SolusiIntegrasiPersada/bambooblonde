...
header_title_new_order = ['Photo', 'Name', 'Color', 'Fabric', 'Sizes', 'Formula', 'Breakdown Sizes', 'Order Qty', 'Taboo Cost Price', 'Minimum Retail', 'Total', 'Delivery Date', 'Notes']
header_title_reorder = ['Photo', 'Name', 'Color', 'Age (Week) Round Up', 'Sizes', 'Qty Sold', 'In Stock', 'Last Order Qty', 'Formula', 'Order', 'Taboo Cost Price', 'Minimum Retail', 'Retail Price', 'Total']
...
class XlsxSampleDevelopment(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        formatHeaderCompanyReOrder = workbook.add_format({'font_size': 14, 'valign':'vcenter', 'align': 'center', 'bold': True})
        formatSubHeaderCompanyReOrder = workbook.add_format({'font_size': 11, 'valign':'vcenter', 'align': 'center', 'bold': True})
        formatHeaderTableReOrder = workbook.add_format({'font_size': 11, 'valign':'vcenter', 'align': 'centre', 'bold': True, 'bg_color':'#8db4e2', 'color':'black', 'text_wrap': True, 'border': 1})
        ...
        for sheet in dictionary_sheet:
            ...
            if i == 1:
                ...
                # ])
                row = 1
                sheet.merge_range(row, 0, row, len(header_title_new_order)-1, f'{datas.get("from_date")} - {datas.get("to_date")} NEW ORDER LIST', formatHeaderCompany)
                ...
                for prl in pw_ids.mapped('product_id.product_tmpl_id'):
                    ...
                    order_qty = 0.0
                    # po_id = prl.env['purchase.order.line'].sudo().search([('product_id.product_tmpl_id', '=', prl.id), ('order_id.state', '=', 'draft'), ('order_id.picking_type_id.barcode', '=', 'WHBB-RECEIPTS')])
                    # po_id = prl.env['purchase.order.line'].sudo().search([('product_id.product_tmpl_id', '=', prl.id), ('order_id.state', 'in', ['purchase', 'done']), ('order_id.picking_type_id.barcode', '=', 'WHBB-RECEIPTS')])
                    # po_id = prl.env['mrp.production'].sudo().search([('product_tmpl_id', '=', prl.id), ('state', 'not in', ['draft', 'cancel']), ('is_sample', '!=', True)])
                    ...
                    name = prl.name
                    color = ', '.join(prl.attribute_line_ids.filtered(lambda x: x.attribute_id.name in list_color).value_ids.mapped('name')) or ''
                    # fabric = pw_2_ids[0].fabric.name or '' if len(pw_2_ids) > 0 else ''
                    fabric = pw_2_ids[0].fabric_por.name or '' if len(pw_2_ids) > 0 else ''
                    sizes = ', '.join(prl.attribute_line_ids.filtered(lambda x: x.attribute_id.name in list_size).value_ids.mapped('name')) or ''
                    formula = ''
                    breakdown_sizes = ''
                    flag_formula = 0
                    ...
                    delivery_date = '' if len(pw_2_ids) < 1 else pw_2_ids[0].order_id.effective_date.strftime('%d/%m/%Y') if pw_2_ids[0].order_id.effective_date else ''
            ...
            if i == 2:
                ...
                row = 1
                sheet.merge_range(row, 0, row, len(header_title_reorder)-1, f'{datas.get("from_date")} - {datas.get("to_date")} REORDER LIST', formatHeaderCompanyReOrder)

                row += 1
                sheet.merge_range(row, 0, row, len(header_title_reorder)-1, f'{datas.get("sales_from_date")} - {datas.get("sales_to_date")} SALES FROM', formatSubHeaderCompanyReOrder)
                ...
                for product_tmpl in pw_ids.mapped('product_id.product_tmpl_id'):
                    ...
                    for line in purchase_pw:
                        ...
                        qty_ppw = sum(line.order_line.filtered(lambda x: x.product_id.product_tmpl_id == product_tmpl).mapped('product_qty')) if len(line.order_line) > 0 else 0
                    ...
                    if row_start != row:
                        if picture:
                            sheet.insert_image(row_start, 0, "image.png", {'image_data': picture, 'object_position': 1, 'x_scale': x_scale, 'y_scale': y_scale, 'x_offset': 10, 'y_offset': 5})
                        ...
                        if picture:
                            sheet.insert_image(row, 0, "image.png", {'image_data': picture, 'object_position': 1, 'x_scale': x_scale, 'y_scale': y_scale, 'x_offset': 10, 'y_offset': 5})
