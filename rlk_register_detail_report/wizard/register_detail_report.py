...
class RegisterDetailReport(models.TransientModel):
    ...
    def print_excel_report(self):
        ...
        where_date = " and 1=1 "
        if self.shift == 'ALL':
            where_date += " and DATE_TRUNC('day', o.date_order) >= '%s' and DATE_TRUNC('day', o.date_order) <= '%s' and s.shift in ('Shift A','Shift B')"%(self.start_period, self.end_period)
        elif self.shift == 'Shift A':
            where_date += " and DATE_TRUNC('day', o.date_order) >= '%s' and DATE_TRUNC('day', o.date_order) <= '%s' and s.shift in ('Shift A')"%(self.start_period, self.end_period)
        elif self.shift == 'Shift B':
            where_date += " and DATE_TRUNC('day', o.date_order) >= '%s' and DATE_TRUNC('day', o.date_order) <= '%s' and s.shift in ('Shift B')"%(self.start_period, self.end_period)
        ...
        query = """
            SELECT 
                s.shift as shift,
                vr.name as region,
                count(o.region_id) as region_count,
                DATE_TRUNC('hour', s.start_at) as start_at, 
                DATE_TRUNC('hour', s.stop_at) as stop_at, 
                o.date_order as date_order, 
                o.pos_reference as invoice, 
                coalesce(sum(o.amount_total),0) as total_sales,
                coalesce(sum(ol.discount),0) as discount,
                CASE 
                    when coalesce(sum(o.amount_total),0) < 0 then coalesce(sum(o.amount_total),0)
                END as total_return,
                CASE 
            ...
            FROM 
                pos_order o
            LEFT JOIN 
                (select ol.order_id, coalesce(sum(ol.price_unit * ol.qty),0) - coalesce(sum(ol.price_subtotal_incl),0) as discount from pos_order_line ol group by ol.order_id) ol on ol.order_id=o.id
            LEFT JOIN 
                pos_session s on o.session_id=s.id
            LEFT JOIN 
                pos_payment p on o.id=p.pos_order_id
            LEFT JOIN 
                pos_payment_method pm on p.payment_method_id=pm.id
            LEFT JOIN 
                visitor_region vr on o.region_id=vr.id
            LEFT JOIN 
                account_journal aj on pm.journal_id=aj.id
            WHERE 
                o.state not in ('draft','cancel') and s.config_id=%s %s
            GROUP BY 
                s.shift, DATE_TRUNC('hour', s.start_at), DATE_TRUNC('hour', s.stop_at), o.date_order, o.pos_reference, o.region_id, vr.name, aj.type
            ORDER BY 
                o.date_order
        """
        ...
        worksheet.merge_range('A8:N8', 'Staf/Cashier : ' + str(cashier), wbf['title_doc2'])
        ...
        if self.env.user.company_id.logo:
            ...
            worksheet.insert_image('N3:P6', "image.png", {'image_data': logo_company, 'bg_color': '#FFFFFF', 'x_scale': resize_scale, 'y_scale': resize_scale})
        ...
        if self.shift == 'ALL':
            ...
            total_sales_per_shift1 = 0
            total_paymentcash_per_shift1 = 0
            total_paymentcc_per_shift1 = 0
            total_discount_per_shift1 = 0
            total_return_per_shift1 = 0
            total_netsales_per_shift1 = 0

            total_sales_per_shift2 = 0
            total_paymentcash_per_shift2 = 0
            total_paymentcc_per_shift2 = 0
            total_discount_per_shift2 = 0
            total_return_per_shift2 = 0
            total_netsales_per_shift2 = 0

            total_sales_per_date = 0
            total_paymentcash_per_date = 0
            total_paymentcc_per_date = 0
            total_discount_per_date = 0
            total_return_per_date = 0
            total_netsales_per_date = 0

            grand_total_sales = 0
            grand_total_paymentcash = 0
            grand_total_paymentcc = 0
            grand_total_discount = 0
            grand_total_return = 0
            grand_total_netsales = 0
            ...
            for res in result :
                ...
                date_order = pytz.UTC.localize(res['date_order']).astimezone(timezone(self.env.user.tz or 'UTC'))
                ...
                amount_return = 0
                if res['total_return']:
                    amount_return = res['total_return']
                net_sales = res['total_sales'] - amount_return
                total_return = res['total_return']
                discount = res['discount']

                if shift == 'Shift A':
                    no_shift1 += 1
                    worksheet.write(row-1, 0, date_order.strftime('%Y-%m-%d %H:%M:%S'), wbf['content'])
                    worksheet.write(row-1, 1, region_name or '', wbf['content2'])
                    worksheet.write(row-1, 2, res['invoice'], wbf['content'])
                    worksheet.write(row-1, 3, total_sales, wbf['content_float'])
                    worksheet.write(row-1, 4, payment_cash, wbf['content_float'])
                    ...
                    worksheet.write(row-1, 10, payment_cc, wbf['content_float'])
                    worksheet.write(row-1, 11, '-', wbf['content2'])
                    worksheet.write(row-1, 12, discount, wbf['content_float'])
                    worksheet.write(row-1, 13, total_return, wbf['content_float'])
                    worksheet.write(row-1, 14, '-', wbf['content2'])
            ...
            row1 = row-1
            worksheet.merge_range('A%s:C%s'%(row,row), 'Total Per Shift A', wbf['total_smoke'])
            worksheet.write(row1, 3, total_sales_per_shift1, wbf['total_float_smoke'])
            worksheet.write(row1, 4, total_paymentcash_per_shift1, wbf['total_float_smoke'])
            ...
            worksheet.write(row1, 10, total_paymentcc_per_shift1, wbf['total_float_smoke'])
            worksheet.write(row1, 11, '-', wbf['total_float_smoke2'])
            worksheet.write(row1, 12, total_discount_per_shift1, wbf['total_float_smoke'])
            worksheet.write(row1, 13, total_return_per_shift1, wbf['total_float_smoke'])
            worksheet.write(row1, 14, '-', wbf['total_float_smoke2'])
            worksheet.write(row1, 15, total_netsales_per_shift1, wbf['total_float_smoke'])
            row1+=2

            worksheet.merge_range('A%s:P%s'%(row1,row1), 'Shift B', wbf['title_doc3'])
            row1 +=1
            ...
            for res in result :
                ...
                date_order = pytz.UTC.localize(res['date_order']).astimezone(timezone(self.env.user.tz or 'UTC'))
                region_count = str(res['region_count']) + ' (' + str(res['region']) + ')'
                total_sales = res['total_sales']
                payment_cash = res['payment_cash']
                payment_cc = res['payment_cc']
                if res['total_sales'] < 0:
                ...
                if shift == 'Shift B':
                    no_shift2 += 1
                    worksheet.write(row1-1, 0, date_order.strftime('%Y-%m-%d %H:%M:%S'), wbf['content'])
                    worksheet.write(row1-1, 1, region_count, wbf['content2'])
                    worksheet.write(row1-1, 2, res['invoice'], wbf['content'])
                    worksheet.write(row1-1, 3, total_sales, wbf['content_float'])
                    worksheet.write(row1-1, 4, payment_cash, wbf['content_float'])
                    ...
                    worksheet.write(row1-1, 10, payment_cc, wbf['content_float'])
                    worksheet.write(row1-1, 11, '-', wbf['content2'])
                    worksheet.write(row1-1, 12, discount, wbf['content_float'])
                    worksheet.write(row1-1, 13, total_return, wbf['content_float'])
                    worksheet.write(row1-1, 14, '-', wbf['content2'])
                    worksheet.write(row1-1, 15, net_sales, wbf['content_float'])
            ...
            row2=row1
            worksheet.merge_range('A%s:C%s'%(row2,row2), 'Total Per Shift B', wbf['total_smoke'])
            worksheet.write(row2-1, 3, total_sales_per_shift2, wbf['total_float_smoke'])
            worksheet.write(row2-1, 4, total_paymentcash_per_shift2, wbf['total_float_smoke'])
            ...
            worksheet.write(row2-1, 10, total_paymentcc_per_shift2, wbf['total_float_smoke'])
            worksheet.write(row2-1, 11, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 12, total_discount_per_shift2, wbf['total_float_smoke'])
            worksheet.write(row2-1, 13, total_return_per_shift2, wbf['total_float_smoke'])
            worksheet.write(row2-1, 14, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 15, total_netsales_per_shift2, wbf['total_float_smoke'])
            row2+=1

            total_sales_per_date = total_sales_per_shift1 + total_sales_per_shift2
            total_paymentcash_per_date = total_paymentcash_per_shift1 + total_paymentcash_per_shift2
            ...
            worksheet.write(row2-1, 8, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 9, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 10, total_paymentcc_per_date, wbf['total_float_smoke'])
            worksheet.write(row2-1, 11, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 12, total_discount_per_date, wbf['total_float_smoke'])
            worksheet.write(row2-1, 13, total_return_per_date, wbf['total_float_smoke'])
            worksheet.write(row2-1, 14, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 15, total_netsales_per_date, wbf['total_float_smoke'])
            row2+=1
            worksheet.merge_range('A%s:C%s'%(row2,row2), 'Grand Total', wbf['total_smoke'])
            worksheet.write(row2-1, 3, grand_total_sales, wbf['total_float_smoke'])
            worksheet.write(row2-1, 4, grand_total_paymentcash, wbf['total_float_smoke'])
            worksheet.write(row2-1, 5, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 6, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 7, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 8, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 9, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 10, grand_total_paymentcc, wbf['total_float_smoke'])
            worksheet.write(row2-1, 11, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 12, grand_total_discount, wbf['total_float_smoke'])
            worksheet.write(row2-1, 13, grand_total_return, wbf['total_float_smoke'])
            worksheet.write(row2-1, 14, '-', wbf['total_float_smoke2'])
            worksheet.write(row2-1, 15, grand_total_netsales, wbf['total_float_smoke'])
    ...
    def add_workbook_format(self, workbook):
        ...
        wbf['header_smoke'] = workbook.add_format({'bold': 1,'align': 'center','bg_color': colors['smoke'],'font_color': '#000000', 'font_name': 'Georgia'})
        ...
        wbf['total_smoke'] = workbook.add_format({'bold':1, 'bg_color':colors['smoke'], 'align':'right', 'font_name': 'Georgia'})
        wbf['total_smoke'].set_left()
        wbf['total_smoke'].set_right()
        wbf['total_smoke'].set_top()
        wbf['total_smoke'].set_bottom()

        wbf['total_float_smoke'] = workbook.add_format({'bold':1, 'bg_color':colors['smoke'], 'align': 'right','num_format': '#,##0', 'font_name': 'Georgia'})
        ...
        wbf['total_float_smoke2'] = workbook.add_format({'bold':1, 'bg_color':colors['smoke'], 'align': 'center','num_format': '#,##0', 'font_name': 'Georgia'})
