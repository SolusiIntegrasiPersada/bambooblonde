...
class StockPicking(models.Model):
    ...
    def consolidate_lines(self):
        ...
        for line in self.move_ids_without_package:
            ...
            key = (model, category)  # Group by model and category

            if key in consolidated_lines:
                if (name, colour) in consolidated_lines[key]:
                    consolidated_lines[key][(name, colour)]['sizes'].append(size)
                else:
                    consolidated_lines[key][(name, colour)] = {
                        'sizes': [size],
                        'model': model,
                        'category': category,
                    }
        ...
        for (model, category), subgroups in consolidated_lines.items():
            for (name, colour), type_data in subgroups.items():
