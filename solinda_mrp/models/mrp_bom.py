...
class MrpBom(models.Model):
    ...
    nominal1 = fields.Float('Nominal1',compute=_compute_nominal)
    nominal2 = fields.Float('Nominal2',compute=_compute_nominal)
    nominal3 = fields.Float('Nominal3',compute=_compute_nominal)
    ...
    def button_bom_line_qty(self):
        bom_line_dict = {}
        for variant_line in self.bom_line_variant_ids:
            bom_line = self.bom_line_ids.filtered(lambda x: x.product_id == variant_line.product_id)
            if bom_line:
                qty_sum = sum(self.bom_line_variant_ids.filtered(lambda x: x.product_id == variant_line.product_id).mapped('product_qty'))
                same_product_count = len(self.bom_line_variant_ids.filtered(lambda x: x.product_id == variant_line.product_id))
        ...
        for bom_line_id, product_qty in bom_line_dict.items():
            # bom_line = self.bom_line_ids.browse(bom_line_id.origin)
            bom_line = self.bom_line_ids.browse(bom_line_id)
            bom_line_list.append((1, bom_line.id, {'total_qty_variant': product_qty}))

        self.bom_line_ids = bom_line_list
    ...
    def _onchange_bom_line_variant_ids(self):
        # if self.product_tmpl_id and not self.bom_line_variant_ids:
