...
class account(models.Model):
    ...
    def _check_duplicate_supplier_reference(self):
        ...
        if not self.env['ir.module.module'].sudo().search(
                [('name', '=', 'vz_purchase_downpayment'), ('state', '=', 'installed')]):
            if duplicated_moves:
...
class PurchaseOrder(models.Model):
    ...
    #     """
    #     for record in self:
    #         if (record.purchase_active_id.advance_payment_method != 'delivered') and ('purchase_orders' in record.env.context):
    #             po_line_values = record.env.context.get('po_line_values')
    #         else:
    ...
    #                                 record.purchase_active_id.advance_payment_method == 'delivered' and not record.purchase_active_id.deduct_down_payments):
    #                             continue
    #                         if pending_section:
    ...
    #
    #         if record.purchase_active_id.advance_payment_method != 'delivered':
    #             if po_line_values:
    ...
    #                     reference = _("Downpayment: %s [%s]" % (record.ref_count, invoice_vals_list[0]['ref']))
