...
class ResPartner(models.Model):
    ...
    def _compute_promo_coupon(self):
        for record in self:
            coupon_program = self.env['coupon.program'].search([('program_type', '=', 'promotion_program')]).filtered(lambda x: record.id in x.valid_partner_ids.ids and 'available_in_pos","=",True' in str(x.rule_products_domain) and x.rule_min_quantity == 1 and x.rule_minimum_amount <= 0)
            if coupon_program:
