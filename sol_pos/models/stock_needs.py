...
class StockLine(models.Model):
  ...
  code = fields.Char(string='Stock ID')
  ...
  stock_id = fields.Many2one(comodel_name='stock.needs', string='stock', ondelete='cascade')
...
class StockNeeds(models.Model):
  _name = 'stock.needs'
  _description = 'Stock Needs'
  _order = "id desc"
  ...
  stock_ids = fields.One2many(comodel_name='stock.line', inverse_name='stock_id', string='stock')
  category_id = fields.Many2one('product.category', string='Category')

  @api.model
  def create(self, vals):
    res = super(StockNeeds, self).create(vals)
    res.name = self.env["ir.sequence"].next_by_code('stock.needs.seq')
    return res  
