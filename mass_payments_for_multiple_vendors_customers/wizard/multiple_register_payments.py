...
class MultipleRegisterPayment(models.TransientModel):
    ...
    selection_amount = fields.Selection(
        [('percentage', 'Percentage'), ('fixed', 'Fixed')], string='Amount Selection')
    percentage_amount = fields.Float(string='Percentage Payment')
    fixed_amount = fields.Float(string='Fixed Payment')
    ####
    ...
    @api.onchange('selection_amount', 'percentage_amount', 'fixed_amount')
    def _onchange_amount(self):
        for record in self:
            if record.percentage_amount and record.selection_amount == 'percentage':
                for line in self.payment_lines:
                    line.amount = line.amount_due * record.percentage_amount
            elif record.fixed_amount and record.selection_amount == 'fixed':
                for line in self.payment_lines:
                    if record.fixed_amount > line.amount_due:
                        raise UserError(_('Selected fixed amount bigger than %s amount due' % line.vendor_bill_name))
                    line.amount = record.fixed_amount
